...
code
TypeScript
type ReviewStatus = 'learning' | 'review' | 'relearning';

interface WordCard {
  id: string;               // UUID
  word: string;             // 目标单词 (e.g., "ephemeral")
  sentence: string;         // 原句 (e.g., "Fashions are ephemeral...")
  
  // 内容字段 (AI生成 + 人工修正)
  meaning_cn: string;       // 中文释义 (e.g., "短暂的")
  mnemonics: string;        // 助记 (e.g., "ephe(day) -> 只有一天的 -> 短暂")
  
  // SRS 核心数据 (SM2算法)
  state: ReviewStatus;
  interval: number;         // 当前间隔 (天)
  ease_factor: number;      // 难度系数 (默认 2.5)
  repetition: number;       // 连续正确次数
  next_review_at: number;   // 下次复习时间戳 (Unix Timestamp)
  
  created_at: number;
}
4.2 AI Prompt 策略 (Prompt Engineering)
建议使用 JSON Mode 以确保后端解析稳定性。
code
Text
System: You are an expert English teacher.
User: 
Analyze the word "{word}" strictly within the context of this sentence: "{sentence}".
Requirements:
1. contextual_meaning: The concise Chinese meaning of the word IN THIS SPECIFIC CONTEXT (max 15 chars).
2. mnemonic: A short aid to memory (etymology or association).
